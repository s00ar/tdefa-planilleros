...
function EditField({
  ...
  return (
    <label>
      <span className="mb-[10px] block text-[14px] font-medium uppercase tracking-[0.04em] text-[#464747]">
        {label}
      </span>
      {children}
    </label>
...
export function AdminPlanilleroEditPage() {
    ...
    (async () => {
      ...
      try {
        ...
        toast.error("No se pudo cargar el planillero", {
          description: loadError instanceof Error ? loadError.message : "Intentá nuevamente.",
        });
  ...
  const save = async () => {
    ...
    if (fullName.length < 3 || username.trim().length < 3) {
      toast.error("Datos incompletos", {
        description: "Ingresá nombre, apellido y usuario.",
      });
      return;
    }
    ...
    try {
      ...
      toast.error("No se pudieron guardar los cambios", {
        description: saveError instanceof Error ? saveError.message : "Intentá nuevamente.",
      });
    } finally {
      setSaving(false);
    }
  ...
  const remove = async () => {
    ...
    try {
      ...
      toast.error("No se pudo eliminar el planillero", {
        description: removeError instanceof Error ? removeError.message : "Intentá nuevamente.",
      });
    }
  };
  ...
  return (
    <div className="space-y-[28px]">
      <div className="flex flex-wrap items-start justify-between gap-[20px]">
        <div>
          <nav className="mb-[14px] flex items-center gap-[10px] text-[14px] font-medium text-[#5e5e5e]">
            ...
            <span className="font-bold text-[#570000]">Edición</span>
          </nav>
          <h1 className="m-0 font-heading text-[38px] font-extrabold leading-[1.08] text-[#300000]">
            Editar planillero
          </h1>
          <p className="mt-[12px] text-[18px] leading-[1.45] text-[#5e5e5e]">
            Modificá los datos persistidos en la base para este registro.
          </p>
        </div>

        <div className="rounded-[10px] bg-[#ffffff] px-[20px] py-[16px] shadow-[0_8px_24px_rgba(36,25,23,0.05)]">
          <p className="text-[12px] font-bold uppercase tracking-[0.14em] text-[#8b716d]">ID de base</p>
          <p className="mt-[8px] text-[16px] font-semibold text-[#300000]">{item.id}</p>
        </div>
      </div>

      <div className="grid gap-[24px] xl:grid-cols-[minmax(0,1fr)_360px]">
        <section className="rounded-[10px] bg-[#ffffff] p-[32px] shadow-[0_10px_30px_rgba(36,25,23,0.05)]">
          <div className="grid gap-[24px] md:grid-cols-2">
            <EditField label="Nombre">
              <input
                className="h-[56px] w-full rounded-[8px] border border-[#ead5d2] bg-[#fff8f7] px-[16px] text-[16px] text-[#241917] outline-none focus:ring-2 focus:ring-[#570000]/15"
        ...
        <aside className="space-y-[24px]">
          <section className="rounded-[10px] bg-[#ffffff] p-[28px] shadow-[0_10px_30px_rgba(36,25,23,0.05)]">
            <div className="flex items-center justify-between gap-[12px]">
              <div>
                <h2 className="m-0 text-[22px] font-semibold text-[#241917]">Estado</h2>
                <p className="mt-[8px] text-[15px] leading-[1.5] text-[#5e5e5e]">
                  Activá o desactivá el acceso del planillero.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-[20px] rounded-[8px] bg-[#fff0ee] px-[16px] py-[14px] text-[14px] font-semibold text-[#852318]">
              {status === "activo" ? "El planillero tiene acceso habilitado." : "El planillero quedó deshabilitado."}
            </div>
          </section>
      ...
      <div className="flex flex-wrap items-center justify-between gap-[16px] border-t border-[#e5d6d3] pt-[24px]">
        <button
          className="flex h-[52px] items-center gap-[10px] rounded-[8px] px-[20px] text-[16px] font-semibold text-[#ba1a1a] transition hover:bg-[#fff0ee]"
          onClick={() => void remove()}
          type="button"
        >
        ...
        <div className="flex items-center gap-[16px]">
          ...
          <button
            className="flex h-[52px] items-center gap-[12px] rounded-[8px] bg-[linear-gradient(135deg,#a53a2d_0%,#570000_100%)] px-[24px] text-[16px] font-semibold text-[#ffffff] shadow-[0_18px_32px_rgba(48,0,0,0.16)] disabled:opacity-60"
            disabled={saving}
            onClick={() => void save()}
            type="button"
          >
